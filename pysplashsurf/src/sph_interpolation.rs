    ...
    ($name: ident, $type: ident) => {
        ...
        impl $name {
            ...
            fn interpolate_vector_quantity<'py>(
                ...
                let particle_quantity: PyReadonlyArray2<$type> = particle_quantity.extract().unwrap();
                ...
                let res_vec = self.inner.interpolate_vector_quantity(particle_quantity, interpolation_points, first_order_correction);
                let res_vec = bytemuck::allocation::cast_vec::<Vector3<$type>, $type>(res_vec);

                let res: &[$type] = res_vec.as_slice();
                let res: ArrayView2<$type> =
                    ArrayView::from_shape((res.len() / 3, 3), res)
                        .unwrap();
